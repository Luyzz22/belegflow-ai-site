...
/* Echte Integrationen, die FlowCheck AI+ unterstützt: DATEV-Export, n8n-Webhooks,
   Stripe-Abrechnung, Anthropic-KI. Logos via 21st.dev (magic MCP); DATEV als Wortmarke. */

function N8n() {
  return (
    <svg viewBox="0 0 228 120" className="h-6 w-auto" role="img" aria-label="n8n">
...
function Stripe() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="100 100 312 312" className="h-7 w-auto" role="img" aria-label="Stripe">
      <path fill="currentColor" fillRule="evenodd" d="m120 392 272-57.683V120l-272 58.357z" clipRule="evenodd" />
    </svg>
  );
}
function Anthropic() {
  return (
    <svg viewBox="0 0 24 24" className="h-6 w-auto" role="img" aria-label="Anthropic" fill="currentColor">
      <path d="M13.827 3.52h3.603L24 20h-3.603l-6.57-16.48zm-7.258 0h3.767L16.906 20h-3.674l-1.343-3.461H5.017l-1.344 3.46H0L6.57 3.522zm4.132 9.959L8.453 7.687 6.205 13.48H10.7z" />
    </svg>
  );
}

const LOGOS: { node: React.ReactNode; name: string }[] = [
  { node: <span className="text-lg font-extrabold tracking-tight">DATEV</span>, name: "DATEV-Export" },
  { node: <N8n />, name: "n8n-Webhooks" },
  { node: <Stripe />, name: "Stripe-Abrechnung" },
  { node: <Anthropic />, name: "Anthropic-KI" },
];

export default function Integrations() {
  return (
    <section className="border-b border-[rgba(0,56,86,0.08)] bg-white py-12">
      <Reveal className="mx-auto max-w-6xl px-6 text-center">
        <p className="text-xs font-semibold uppercase tracking-wider text-[#64748b]">
          Integriert in Ihren Finanz-Stack
        </p>
        <div className="mt-7 flex flex-wrap items-center justify-center gap-x-12 gap-y-6">
          {LOGOS.map((l) => (
            <div
              key={l.name}
              title={l.name}
              className="flex items-center gap-2 text-[#94a3b8] grayscale transition-all duration-300 hover:text-[#003856] hover:grayscale-0"
            >
              {l.node}
            </div>
          ))}
        ...
        <p className="mt-6 text-xs text-[#94a3b8]">
          DATEV-Buchungsstapel · n8n-Automationen · Stripe-Abrechnung · Anthropic-KI
        </p>
      </Reveal>
    </section>
  );
