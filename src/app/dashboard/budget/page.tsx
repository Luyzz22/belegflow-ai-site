        ...
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div><h1 className="text-base font-semibold leading-tight">Budget</h1><p className="text-[11px] text-gray-400">Jahresplanung {jahr}</p></div>
            </div>
          </div>
          <div className="flex gap-2 items-center">
            {[2025,2026,2027].map(y=>(<button key={y} onClick={()=>setJahr(y)} className={`px-3 py-1.5 rounded-md text-xs font-medium transition ${jahr===y?"bg-[#e85d04] text-gray-900":"text-gray-500 hover:text-gray-900 bg-white border border-gray-200"}`}>{y}</button>))}
          </div>
        </div>
      </div>
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        {loading ? (<div className="flex justify-center py-24"><div className="flex gap-1.5">{[0,150,300].map(d=>(<div key={d} className="w-2.5 h-2.5 bg-[#e85d04] rounded-full animate-bounce" style={{animationDelay:`${d}ms`}} />))}</div></div>) : (<>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            ].map((k,i)=>(<div key={i} className={`bg-gradient-to-br ${k.a} border border-gray-200 rounded-2xl p-5 shadow-sm`}><p className="text-[11px] text-gray-500 uppercase tracking-wider font-medium">{k.l}</p><p className="text-xl font-bold mt-1.5">{k.v}</p></div>))}
          </div>

          {/* Budget vs Actual Chart */}
          {chartData.length > 0 && (<div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
            <h3 className="text-sm font-semibold text-gray-600 mb-5">Budget vs. Ist nach Kategorie</h3>
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={chartData} layout="vertical"><CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" horizontal={false} /><XAxis type="number" stroke="#9ca3af" tick={{fontSize:10,fill:"#525252"}} /><YAxis type="category" dataKey="name" stroke="#9ca3af" tick={{fontSize:11,fill:"#737373"}} width={100} /><Tooltip contentStyle={{background:"#171717",border:"1px solid #262626",borderRadius:8,fontSize:12}} /><Bar dataKey="budget" fill="#6366f1" radius={[0,4,4,0]} barSize={12} name="Budget" /><Bar dataKey="actual" fill="#e85d04" radius={[0,4,4,0]} barSize={12} name="Ist" /></BarChart>
            </ResponsiveContainer>
          </div>)}
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-sm font-semibold text-gray-600">Monatsbudget bearbeiten</h3>
              <div className="flex gap-1 flex-wrap">
                {MONATE.map((m,i)=>(<button key={i} onClick={()=>setSelMonat(i+1)} className={`px-2 py-1 rounded text-[10px] font-medium transition ${selMonat===i+1?"bg-[#e85d04] text-gray-900":"text-gray-400 hover:text-gray-900 bg-gray-50 border border-gray-200"}`}>{m}</button>))}
              </div>
            </div>
            <div className="space-y-2">
              <div className="hidden sm:grid grid-cols-12 gap-4 px-3 py-2 text-[10px] text-gray-300 uppercase tracking-widest">
              ...
              {monatData.map((m:any)=>(<div key={m.kategorie_id} className="grid grid-cols-12 gap-4 px-3 py-2.5 rounded-lg hover:bg-gray-50 transition items-center">
                <div className="col-span-4 text-sm">{m.name}</div>
                <div className="col-span-3 text-xs text-gray-400 font-mono">{kategorien.find((k:any)=>k.id===m.kategorie_id)?.konten_mapping||""}</div>
                <div className="col-span-3 text-right">
                  {editKat===m.kategorie_id ? (<input type="number" value={editVal} onChange={e=>setEditVal(e.target.value)} onKeyDown={e=>e.key==="Enter"&&saveBudget(m.kategorie_id)} className="w-24 bg-[#f4f7fa] border border-[#e85d04] rounded px-2 py-1 text-sm text-right text-gray-900 focus:outline-none" autoFocus />) : (<span className="text-sm font-medium">{m.betrag.toLocaleString("de-DE",{style:"currency",currency:"EUR"})}</span>)}
                </div>
                <div className="col-span-2 text-right">
                  {editKat===m.kategorie_id ? (<button onClick={()=>saveBudget(m.kategorie_id)} className="text-xs text-emerald-400 hover:text-emerald-300">Speichern</button>) : (<button onClick={()=>{setEditKat(m.kategorie_id);setEditVal(String(m.betrag));}} className="text-xs text-gray-400 hover:text-gray-900">Bearbeiten</button>)}
                </div>
              </div>))}
            </div>
          </div>
