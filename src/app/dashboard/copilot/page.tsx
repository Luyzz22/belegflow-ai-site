        ...
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div><h1 className="text-lg font-semibold">Finance Copilot</h1><p className="text-xs text-[#737373]">KI-Finanzassistent</p></div>
            </div>
          </div>
        ...
        <div className="py-6 space-y-6">
          {messages.map((msg, i) => (
            <div key={i} className={"flex " + (msg.role === "user" ? "justify-end" : "justify-start")}>
              <div className={"max-w-[85%] rounded-2xl px-5 py-4 " + (msg.role === "user"
                ? "bg-[#e85d04]/20 border border-[#e85d04]/30 text-[#d4d4d4]"
                : "bg-[#171717]/80 border border-[#262626] text-[#d4d4d4]")}>
                {msg.role === "assistant" && (
                  <div className="flex items-center gap-2 mb-2 pb-2 border-b border-[#262626]">
                    <span className="text-xs text-[#525252] font-medium">Finance Copilot</span>
                    {msg.model && <span className="text-xs px-2 py-0.5 rounded-full bg-[#262626] text-[#737373]">{msg.model}</span>}
                  </div>
                )}
                <div className="leading-relaxed text-sm" dangerouslySetInnerHTML={{ __html: renderMd(msg.content) }} />
                {msg.sources && (
                  <div className="mt-3 pt-2 border-t border-[#262626] flex gap-3 flex-wrap">
                    <span className="text-xs text-[#525252]">📊 {msg.sources.invoices_analyzed} Rechnungen</span>
                    <span className="text-xs text-[#525252]">📋 {msg.sources.events_analyzed} Events</span>
                    <span className="text-xs text-[#525252]">🤖 {msg.sources.kontierungen_analyzed} Kontierungen</span>
                  </div>
                )}
                {msg.suggestions && msg.suggestions.length > 0 && (
                  <div className="mt-3 pt-2 border-t border-[#262626] flex flex-col gap-2">
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-[#171717]/80 border border-[#262626] rounded-2xl px-5 py-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-[#525252] ml-2">analysiert Daten...</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="max-w-5xl mx-auto px-6 py-4">
          <div className="flex gap-3 items-end">
            <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)}
              ...
              className="flex-1 bg-[#171717] border border-[#404040] rounded-xl px-4 py-3 text-sm text-white placeholder-[#525252] resize-none focus:outline-none focus:border-[#e85d04] transition" />
